...
function get_layout_html() {
    ...
    <div class="card p-3 mb-3">
        <h4>Items (Aggregated from selected Sales Orders)</h4>
...
function get_year_options() { let html=""; for(let y=2020;y<=2030;y++) html+=`<option>${y}</option>`; return html; }

// ---------------- load dropdowns ----------------
function load_customers() {
    frappe.db.get_list("Customer", { fields: ["name"], limit: 9999 })
        .then(res => res.forEach(r => $("#filter-customer").append(`<option>${r.name}</option>`)));
}
function load_sales_order_dropdown() {
    frappe.db.get_list("Sales Order", { fields: ["name"], limit: 9999 })
        .then(res => res.forEach(r => $("#filter-sales-order").append(`<option>${r.name}</option>`)));
}
...
let salesOrdersData = []; let salesOrdersPage = 0;
let itemsData = []; let itemsPage = 0;
let bomsData = []; let bomsPage = 0;
let wosData = []; let wosPage = 0;
let jcsData = []; let jcsPage = 0;
...
function set_to_array(s) { return Array.from(s); }
...
function load_sales_orders(reset = false) {
    ...
    if (reset) {
        salesOrdersData = []; salesOrdersPage = 0;
    }
    ...
    const search_filters = {
        ...
        month: $("#filter-month").val() ? $("#filter-month").val() + "-" + String($("#filter-year").val()).slice(-2) : null
    };
...
function renderSalesOrdersPage(resetRender=false) {
    ...
    $(".so-checkbox").off("change").on("change", function () {
    ...
    if (salesOrdersPage + 1 < totalPages) {
        $("#sales-order-load-more").html(`<button class="btn btn-sm btn-outline-primary" onclick="salesOrdersPage++; renderSalesOrdersPage();">Load more Sales Orders</button>`);
    } else {
        $("#sales-order-load-more").empty();
    }
}
function select_all_so_on_page() { $("#sales-order-list .so-checkbox").prop("checked", true).trigger("change"); }
function unselect_all_so_on_page() { $("#sales-order-list .so-checkbox").prop("checked", false).trigger("change"); }
function toggle_and_reload_items(so) {
    const cb = $(`.so-checkbox[data-so='${so}']`);
    cb.prop("checked", !cb.prop("checked")).trigger("change");
}

// ---------------- LEVEL 2: ITEMS aggregated from selected SOs (client pagination) ----------------
function load_items_for_selected_sos(reset=false) {
    const so_list = set_to_array(selected_sales_orders);
    if (!so_list.length) {
        $("#item-controls, #item-list").empty();
        itemsData = []; itemsPage = 0; selected_item_keys.clear();
        $("#bom-list,#rm-list,#jc-list").empty();
        return;
    }

    if (reset) { itemsData = []; itemsPage = 0; }
    ...
    // if already fetched, simply render; otherwise fetch
    if (itemsData.length) {
        renderItemsPage();
        return;
    }
...
function renderItemsPage() {
    ...
    const pageSlice = itemsData.slice(0, endIndex); // progressive append behavior

    let html = `<div class="table-scroll-container"><table class="table table-bordered table-sm"><thead><tr>
        <th>Select</th><th>Sales Order</th><th>Item</th><th>Order Qty</th><th>Rate</th><th>BOM</th><th>Warehouse</th><th>Available</th><th>Produced</th><th>Balance Produce</th><th>Balance Deliver</th>
    </tr></thead><tbody>`;

    pageSlice.forEach((row, idx) => {
        const key = `${row.so_name}||${row.item_code}||${idx}`;
        const checked = selected_item_keys.has(key) ? "checked" : "";
        html += `<tr onclick="open_boms_for_row('${row.item_code}', ${row.order_qty})">
            <td onclick="event.stopPropagation()"><input type="checkbox" class="item-row-chk" data-key="${key}" data-row='${escapeHtml(JSON.stringify(row))}' ${checked}></td>
            <td>${row.so_name}</td>
    ...
    $(".item-row-chk").off("change").on("change", function () {
        const key = $(this).data("key");
        if ($(this).prop("checked")) selected_item_keys.add(key);
        else selected_item_keys.delete(key);
    ...
    if (itemsPage + 1 < totalPages) {
        $("#item-load-more").html(`<button class="btn btn-sm btn-outline-primary" onclick="itemsPage++; renderItemsPage();">Load more Items</button>`);
    } else {
    ...
    // clear lower levels
    $("#bom-list,#rm-list,#jc-list").empty();
    selected_boms.clear(); selected_rm_items.clear();
}
function select_all_items_on_page(){ $("#item-list .item-row-chk").prop("checked", true).trigger("change"); }
function unselect_all_items_on_page(){ $("#item-list .item-row-chk").prop("checked", false).trigger("change"); }
function open_boms_for_row(item_code, qty) { load_boms_for_selected_items([{ item_code, required_for_selected_qty: qty }]); }

// ---------------- LEVEL 3: BOMs aggregated from selected Items (with pagination) ----------------
function show_boms_for_selected_items() {
    const selected_rows = [];
    $(".item-row-chk:checked").each(function () {
        const row = JSON.parse(unescapeHtml($(this).data("row")));
        selected_rows.push({ item_code: row.item_code, required_for_selected_qty: Number(row.order_qty || 0), so: row.so_name, bom_no: row.bom_no || null });
    });
    if (!selected_rows.length) { frappe.msgprint("Select one or more items"); return; }
    load_boms_for_selected_items(selected_rows, true);
}

function load_boms_for_selected_items(selected_rows, reset=false) {
    if (reset) { bomsData = []; bomsPage = 0; }
    $("#bom-controls").html(`
    ...
    frappe.call({
        ...
        callback: function (r) {
            // normalize: API should return array of {item_code,bom_no,bom_qty,required_for_selected_qty,rm_items}
            bomsData = r.message || [];
            bomsPage = 0;
            renderBOMsPage();
        }
...
function renderBOMsPage() {
    ...
    let html = `<table class="table table-bordered table-sm"><thead><tr>
        <th>Select</th><th>Item</th><th>BOM No</th><th>BOM Qty</th><th>Required For Selected Qty</th>
    </tr></thead><tbody>`;

    pageSlice.forEach(b => {
        const checked = selected_boms.has(b.bom_no) ? "checked" : "";
        html += `<tr>
            <td onclick="event.stopPropagation()"><input type="checkbox" class="bom-chk" data-bom='${escapeHtml(JSON.stringify(b))}' data-bomno="${b.bom_no}" ${checked}></td>
            <td>${b.item_code}</td>
            <td>${b.bom_no}</td>
            <td>${b.bom_qty}</td>
    ...
    $(".bom-chk").off("change").on("change", function () {
        const bomno = $(this).data("bomno");
        if ($(this).prop("checked")) selected_boms.add(bomno); else selected_boms.delete(bomno);
    ...
    if (bomsPage + 1 < totalPages) {
        $("#bom-load-more").html(`<button class="btn btn-sm btn-outline-primary" onclick="bomsPage++; renderBOMsPage();">Load more BOMs</button>`);
    } else {
        $("#bom-load-more").empty();
    }

    // clear rm panel when bom list changes
    $("#rm-list").empty(); selected_rm_items.clear();
}
function select_all_boms_on_page(){ $("#bom-list .bom-chk").prop("checked", true).trigger("change"); }
function unselect_all_boms_on_page(){ $("#bom-list .bom-chk").prop("checked", false).trigger("change"); }
...
function show_rm_for_selected_boms(reset=false) {
    ...
    selected_boms.forEach(bn => {
        ...
        if (found) {
            bom_list.push({ bom_no: found.bom_no, required_for_selected_qty: found.required_for_selected_qty || 0 });
        } else {
    ...
    if (!bom_list.length) { frappe.msgprint("Select BOMs first"); return; }
...
function renderRMPage(pageIndex) {
    if (!window._rmResults) { $("#rm-list").html("<p>No RM data.</p>"); return; }
    ...
    pageSlice.forEach(row => {
        ...
        html += `<tr>
            <td onclick="event.stopPropagation()"><input type="checkbox" class="rm-chk" data-rm='${escapeHtml(JSON.stringify(row))}' data-rmcode="${row.rm_item_code}" ${checked}></td>
    ...
    $(".rm-chk").off("change").on("change", function () {
        const rc = $(this).data("rmcode");
        if ($(this).prop("checked")) selected_rm_items.add(rc); else selected_rm_items.delete(rc);
    ...
    if (pageIndex + 1 < totalPages) {
        $("#rm-load-more").html(`<button class="btn btn-sm btn-outline-primary" onclick="renderRMPage(window._rmPageIndex + 1)">Load more RM</button>`);
    } else {
        $("#rm-load-more").empty();
    }
}
function select_all_rm_on_page(){ $("#rm-list .rm-chk").prop("checked", true).trigger("change"); }
function unselect_all_rm_on_page(){ $("#rm-list .rm-chk").prop("checked", false).trigger("change"); }

// ---------------- LEVEL 5: WORK ORDERS aggregated from selected SOs ----------------
function load_work_orders_for_selected_sos(reset=false) {
    const so_list = set_to_array(selected_sales_orders);
    if (!so_list.length) { $("#wo-list").empty(); wosData = []; wosPage = 0; selected_work_orders.clear(); return; }

    if (reset) { wosData = []; wosPage = 0; }
    ...
    if (wosData.length) { renderWOsPage(); return; }
...
function renderWOsPage() {
    ...
    pageSlice.forEach(w => {
        ...
        html += `<tr onclick="toggle_wo_and_load_jcs('${w.wo_name}')">
            <td onclick="event.stopPropagation()"><input type="checkbox" class="wo-chk" data-wo='${escapeHtml(JSON.stringify(w))}' data-woname="${w.wo_name}" ${checked}></td>
            <td>${w.wo_name}</td><td>${w.production_item}</td><td>${w.wo_qty}</td><td>${w.produced_qty}</td>
        </tr>`;
    });
    html += `</tbody></table>`;
    $("#wo-list").html(html);

    $(".wo-chk").off("change").on("change", function () {
        const woname = $(this).data("woname");
        if ($(this).prop("checked")) selected_work_orders.add(woname); else selected_work_orders.delete(woname);
    ...
    if (wosPage + 1 < totalPages) {
        $("#wo-load-more").html(`<button class="btn btn-sm btn-outline-primary" onclick="wosPage++; renderWOsPage();">Load more WOs</button>`);
    } else { $("#wo-load-more").empty(); }
}
function select_all_wos_on_page(){ $("#wo-list .wo-chk").prop("checked", true).trigger("change"); }
function unselect_all_wos_on_page(){ $("#wo-list .wo-chk").prop("checked", false).trigger("change"); }
function toggle_wo_and_load_jcs(wo){ const cb = $(`.wo-chk[data-woname='${wo}']`); cb.prop("checked", !cb.prop("checked")).trigger("change"); load_job_cards_for_selected_wos(true); }
...
function load_job_cards_for_selected_wos(reset=false) {
    ...
    if (!wo_list.length) { $("#jc-list").empty(); jcsData = []; jcsPage = 0; selected_job_cards.clear(); return; }

    if (reset) { jcsData = []; jcsPage = 0; }
    ...
    if (jcsData.length) { renderJCsPage(); return; }
...
function renderJCsPage() {
    ...
    pageSlice.forEach(rw => {
        ...
        html += `<tr>
            <td onclick="event.stopPropagation()"><input type="checkbox" class="jc-chk" data-jc='${escapeHtml(JSON.stringify(rw))}' data-jcname="${rw.job_card}" ${checked}></td>
            <td>${rw.job_card}</td><td>${rw.job_card_status}</td><td>${rw.operation}</td><td>${rw.workstation}</td>
            <td>${rw.rm_item_code}</td><td>${rw.required_qty}</td><td>${rw.available_qty}</td><td>${rw.consumed_qty}</td>
        </tr>`;
    });
    html += `</tbody></table></div>`;
    $("#jc-list").html(html);

    $(".jc-chk").off("change").on("change", function () {
        const jcname = $(this).data("jcname");
        if ($(this).prop("checked")) selected_job_cards.add(jcname); else selected_job_cards.delete(jcname);
    ...
    if (jcsPage + 1 < totalPages) {
        $("#jc-load-more").html(`<button class="btn btn-sm btn-outline-primary" onclick="jcsPage++; renderJCsPage();">Load more Job Cards</button>`);
    } else { $("#jc-load-more").empty(); }
}
function select_all_jc_on_page(){ $("#jc-list .jc-chk").prop("checked", true).trigger("change"); }
function unselect_all_jc_on_page(){ $("#jc-list .jc-chk").prop("checked", false).trigger("change"); }
...
// ---------------- external getters ----------------
function getSelectedSalesOrders(){ return set_to_array(selected_sales_orders); }
function getSelectedItemRows(){ const rows=[]; $(".item-row-chk:checked").each(function(){ rows.push(JSON.parse(unescapeHtml($(this).data("row")))); }); return rows; }
function getSelectedBOMs(){ return set_to_array(selected_boms); }
function getSelectedRMItems(){ return set_to_array(selected_rm_items); }
function getSelectedWOs(){ return set_to_array(selected_work_orders); }
function getSelectedJobCards(){ return set_to_array(selected_job_cards); }

// ---------------- init ----------------
$(document).ready(function(){ /* initial load handled in on_page_load */ });
